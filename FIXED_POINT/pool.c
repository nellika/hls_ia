    ...
    for (i = 0; i < CONV1_NBOUTPUT; i++){
      ...
      for (h = 0; h < CONV1_HEIGHT; h+=2){
        ...
        for (w = 0; w < CONV1_WIDTH; w+=2){
            maxPool=maxPoolingF((short []){input[i][h][w],input[i][h+1][w],input[i][h][w+1],input[i][h+1][w+1]});
    ...
    for (j = 0; j < CONV2_NBOUTPUT; j++){
      ...
      for (h = 0; h < CONV2_HEIGHT; h+=2){
        ...
        for (w = 0; w < CONV2_WIDTH; w+=2){
            maxPool=maxPoolingF((short []){input[j][h][w],input[j][h+1][w],input[j][h][w+1],input[j][h+1][w+1]});
