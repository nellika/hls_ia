...
void WriteBiases(char *filename, short bias[FC2_NBOUTPUT]) {
  ...
  fprintf (weight_file, "short FC2_BIAS[FC2_NBOUTPUT] = { ");
  for (i = 0; i < FC2_NBOUTPUT; i++) {
	  fprintf(weight_file, "%d, ", bias[i]);   
  }
  fprintf (weight_file, "}; \n");
...
void WriteWeightsFc2(char *filename, short weight[FC2_NBOUTPUT][FC1_NBOUTPUT]) {
  ...
  fprintf (weight_file, "short FC2_KERNEL[FC2_NBOUTPUT][FC1_NBOUTPUT] = { \n");
  for (i = 0; i < FC2_NBOUTPUT; i++) {
    fprintf (weight_file, "{ ");
    for (j = 0; j < FC1_NBOUTPUT; j++)
	  fprintf(weight_file, "%d, ", weight[i][j]); 
	fprintf (weight_file, "}, \n");
...
void WriteWeightsFc1(char *filename, short weight[FC1_NBOUTPUT][POOL2_NBOUTPUT][POOL2_HEIGHT][POOL2_WIDTH]) {
  ...
  fprintf (weight_file, "short FC1_KERNEL[FC1_NBOUTPUT][POOL2_NBOUTPUT][POOL2_HEIGHT][POOL2_WIDTH] = { \n");
  for (i = 0; i < FC1_NBOUTPUT; i++) {
    fprintf (weight_file, "{ \n");
    for (j = 0; j < POOL2_NBOUTPUT; j++) {
      fprintf (weight_file, "{ \n");
      for (k = 0; k < POOL2_HEIGHT; k++) {
		fprintf (weight_file, "{ "); 
        for (l = 0; l < POOL2_WIDTH; l++)
...
//conv2_kernel[CONV2_NBOUTPUT][POOL1_NBOUTPUT][CONV2_DIM][CONV2_DIM]
void WriteWeightsConv2(char *filename, short weight[CONV2_NBOUTPUT][POOL1_NBOUTPUT][CONV2_DIM][CONV2_DIM]) {
  ...
  fprintf (weight_file, "short CONV2_KERNEL[CONV2_NBOUTPUT][POOL1_NBOUTPUT][CONV2_DIM][CONV2_DIM] = { \n");
  for (i = 0; i < CONV2_NBOUTPUT; i++) {
    fprintf (weight_file, "{ \n");
    for (j = 0; j < POOL1_NBOUTPUT; j++) {
      fprintf (weight_file, "{ \n");
      for (k = 0; k < CONV2_DIM; k++) {
		fprintf (weight_file, "{ "); 
        for (l = 0; l < CONV2_DIM; l++)
	  	  fprintf(weight_file, "%d, ", weight[i][j][k][l]); 
	    fprintf (weight_file, "}, ");
      }
   	  fprintf (weight_file, "}, \n");
